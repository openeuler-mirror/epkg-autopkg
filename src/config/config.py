...
class BuildConfig:
    ...
    # contains patterns for parsing build.log for missing dependencies
    failed_pats = [
        (r"    !  ([a-zA-Z:]+) is not installed", 'perl'),
        (r"    ([a-zA-Z]+\:\:[a-zA-Z]+) not installed", None),
        (r"(?:-- )?(?:Could|Did) (?:NOT|not) find ([a-zA-Z0-9_-]+)", None),
        (r" ([a-zA-Z0-9\-]*\.m4) not found", None),
        (r" exec: ([a-zA-Z0-9\-]+): not found", None),
        (r"([a-zA-Z0-9\-\_\.]*)\: command not found", None),
        (r"([a-zA-Z\-]*) (?:validation )?tool not found or not executable", None),
        (r"([a-zA-Z\-]+) [0-9\.]+ is required to configure this module; "
         r"please install it or upgrade your CPAN\/CPANPLUS shell.", None),
        (r"-- (.*) not found.", None),
        (r".* /usr/bin/([a-zA-Z0-9-_]*).*not found", None),
        (r".*\.go:.*cannot find package \"(.*)\" in any of:", 'go'),
        (r"/usr/bin/env\: (.*)\: No such file or directory", None),
        (r"/usr/bin/python.*\: No module named (.*)", None),
        (r"Add the installation prefix of \"(.*)\" to CMAKE_PREFIX_PATH", None),
        (r"By not providing \"([a-zA-Z0-9]+).cmake\" in CMAKE_MODULE_PATH this project", None),
        (r"C library '(.*)' not found", None),
        (r"CMake Error at cmake\/modules\/([a-zA-Z0-9]+).cmake", None),
        (r"Can't locate [a-zA-Z0-9_\-\/\.]+ in @INC \(you may need to install the ([a-zA-Z0-9_\-:]+) module\)",
         'perl'),
        (r"Cannot find ([a-zA-Z0-9\-_\.]*)", None),
        (r"Checking for (.*?)\.\.\.no", None),
        (r"Checking for (.*?)\s*: not found", None),
        (r"Checking for (.*?)\s>=.*\s*: not found", None),
        (r"Could not find suitable distribution for Requirement.parse\('([a-zA-Z\-\.]*)", None),
        (r"Download error on https://pypi.python.org/simple/([a-zA-Z0-9\-\._:]+)/", 'pypi'),
        (r"Downloading https?://.*\.python\.org/packages/.*/.?/([A-Za-z]*)/.*", None),
        (r"ERROR: dependencies ['‘]([a-zA-Z0-9\-\.]*)['’].* are not available for package ['‘].*['’]", 'R'),
        (r"ERROR: dependencies ['‘].*['’], ['‘]([a-zA-Z0-9\-\.]*)['’],.* are not available for package ['‘].*['’]",
         'R'),
        (r"ERROR: dependencies.*['‘]([a-zA-Z0-9\-\.]*)['’] are not available for package ['‘].*['’]", 'R'),
        (r"ERROR: dependency ['‘]([a-zA-Z0-9\-\.]*)['’] is not available for package ['‘].*['’]", 'R'),
        (r"Error: Unable to find (.*)", None),
        (r"Error: package ['‘]([a-zA-Z0-9\-\.]*)['’] required by", 'R'),
        (r"ImportError:.* No module named '?([a-zA-Z0-9\-\._]+)'?", 'pypi'),
        (r"ImportError\: ([a-zA-Z]+) module missing", None),
        (r"ImportError\: (?:No module|cannot import) named? (.*)", None),
        (r"ModuleNotFoundError.*No module named '?(.*)'?", 'pypi'),
        (r"Native dependency '(.*)' not found", "pkgconfig"),
        (r"No library found for -l([a-zA-Z\-])", None),
        (r"No (?:matching distribution|local packages or working download links) found for ([a-zA-Z0-9\-\.\_]+)",
         'pypi'),
        (r"No package '([a-zA-Z0-9\-:]*)' found", 'pkgconfig'),
        (r"No rule to make target `(.*)',", None),
        (r"Package (.*) was not found in the pkg-config search path.", 'pkgconfig'),
        (r"Package '([a-zA-Z0-9\-:]*)', required by '.*', not found", 'pkgconfig'),
        (r"Package which this enhances but not available for checking: ['‘]([a-zA-Z0-9\-]*)['’]", 'R'),
        (r"Perhaps you should add the directory containing `([a-zA-Z0-9\-:]*)\.pc'", 'pkgconfig'),
        (r"Program (.*) found: NO", None),
        (r"Target '[a-zA-Z0-9\-]' can't be generated as '(.*)' could not be found", None),
        (r"Unable to `import (.*)`", None),
        (r"Unable to find '(.*)'", None),
        (r"Warning: prerequisite ([a-zA-Z:]+) [0-9\.]+ not found.", 'perl'),
        (r"Warning\: no usable ([a-zA-Z0-9]+) found", None),
        (r"You need ([a-zA-Z0-9\-\_]*) to build this program.", None),
        (r"[Dd]ependency (.*) found: NO \(tried pkgconfig(?: and cmake)?\)", 'pkgconfig'),
        (r"[Dd]ependency (.*) found: NO", None),
        (r"(?:\/usr)?\/bin\/ld: cannot find (-l[a-zA-Z0-9\_]+)", None),
        (r"^.*By not providing \"Find(.*).cmake\" in CMAKE_MODULE_PATH this.*$", None),
        (r"^.*Could not find a package configuration file provided by \"(.*)\".*$", None),
        (r"^.*\"(.*)\" with any of the following names.*$", None),
        (r"[Cc]hecking for (.*) (?:support|development files|with pkg-config)?\.\.\. [Nn]o", None),
        (r"[Cc]hecking pkg-config for (.*?)\.\.\. [Nn]o", 'pkgconfig'),
        (r"checking (.*?)\.\.\. no", None),
        (r"checking for (.*) in default path\.\.\. not found", None),
        (r"checking for (.*)... configure: error", None),
        (r"checking for (.*?)\.\.\. no", None),
        (r"checking for [a-zA-Z0-9\_\-]+ in (.*?)\.\.\. no", None),
        (r"checking for library containing (.*)... no", None),
        (r"checking for perl module ([a-zA-Z:]+) [0-9\.]+... no", 'perl'),
        (r"configure: error: (?:pkg-config missing|Unable to locate) (.*)", None),
        (r"configure: error: ([a-zA-Z0-9]+) (?:is required to build|not found)", None),
        (r"configure: error: Cannot find (.*)\. Make sure", None),
        (r"fatal error\: (.*)\: No such file or directory", None),
        (r"make: ([a-zA-Z0-9].+): (?:Command not found|No such file or directory)", None),
        (r"meson\.build\:[\d]+\:[\d]+\: ERROR: C(?: shared or static)? library \'(.*)\' not found", None),
        (r"there is no package called ['‘]([a-zA-Z0-9\-\.]*)['’]", 'R'),
        (r"unable to execute '([a-zA-Z\-]*)': No such file or directory", None),
        (r"warning: failed to load external entity \"(/usr/share/sgml/docbook/xsl-stylesheets)/.*\"", None),
        (r"which\: no ([a-zA-Z\-]*) in \(", None),
        (r"you may need to install the ([a-zA-Z0-9_\-:\.]*) module", 'perl'),
        (r"(a-zA-Z0-9\-) not found (re-run dependencies script to install)", None),
        (r"error: ([a-zA-Z\-]*)invalid attempt.*in symbol.*", "flags"),
        (r"(CMake Error .* CMakeLists.txt).*", "cmake"),
        (r"Plugin ([a-zA-Z\-.:]+):([0-9.]+) or one of its dependencies could not be resolved", 'java-plugin'),
        (r"Unable to generate requires on unresolvable artifacts: ([a-zA-Z0-9-.:]+(, ){0,1}){1,}", 'java-plugins'),
        (r"Could not resolve dependencies for project ([a-zA-Z.:]+):jar:([0-9.]+): Cannot access ([a-zA-Z-]+)"
         ...
         r" downloaded from it before", 'java-jar'),
    ]
    cmake_params = [
    ...
    patterns = [
        ...
        (r"^/usr/lib64/[a-zA-Z0-9._+-]*\.so$", "lib"),
        (r"^/usr/lib32/[a-zA-Z0-9._+-]*\.so$", 'lib32'),
        (r"^/usr/lib64/glibc-hwcaps/x86-64-v[0-9]+/[a-zA-Z0-9._+-]*\.so$", "lib"),
        (r"^/usr/lib64/haswell/avx512_1/[a-zA-Z0-9._+-]*\.so$", "lib"),
        (r"^/usr/lib64/haswell/[a-zA-Z0-9._+-]*\.so$", "lib"),
        (r"^/usr/lib64/haswell/avx512_1/[a-zA-Z0-9._+-]*\.so$", "lib"),
        ...
        (r"^/usr/share/doc/[a-zA-Z0-9._+-]*/", "doc", "/usr/share/doc/*"),
        (r"^/usr/share/doc/", "doc"),
